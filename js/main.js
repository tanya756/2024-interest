...
const app = Vue.createApp({
    ...
    methods:{
        ...
        game() {
            var role = $('.role'),
                brick1 = $('.brick div:eq(0)'),
                brick2 = $('.brick div:eq(1)'),
                _this = this;

            role.addClass('jump');

            var role_width = role.width(),
                role_height = role.height(),
                role_x = role.offset().left,
                role_y = role.offset().top;

            var brick_width = brick1.width(),
                brick_height = brick1.height(),
                brick1_x = brick1.offset().left,
                brick1_y = brick1.offset().top,
                brick2_x = brick2.offset().left,
                brick2_y = brick2.offset().top;
            ...
            function isCollision(role_x, role_y, role_width, role_height, brick_x, brick_y, brick_width, brick_height) {
                return !(role_x + role_width < brick_x ||
                         role_x > brick_x + brick_width ||
                         role_y + role_height < brick_y ||
                         role_y > brick_y + brick_height);
            }

            let hitBrick1 = isCollision(role_x, role_y, role_width, role_height, brick1_x, brick1_y, brick_width, brick_height);
            let hitBrick2 = isCollision(role_x, role_y, role_width, role_height, brick2_x, brick2_y, brick_width, brick_height);

            
            if (hitBrick1) {
                console.log('撞到了brick1！');
                this.brick1Hit = true;
                // showPopup('#popup1');
                brick1.addClass('hit');
                setTimeout(() => {
                    _this.brick1Hit = false;
                    brick1.removeClass('hit');
                    role.removeClass('jump');
                }, 300); // 0.3秒後移除 hit class 並重置位置
            } 
            if (hitBrick2) {
                console.log('撞到了brick2！');
                this.brick2Hit = true;
                // showPopup('#popup2');
                brick2.addClass('hit');
                setTimeout(() => {
                    _this.brick2Hit = false;
                    brick2.removeClass('hit');
                    role.removeClass('jump');
                }, 300); // 0.3秒後移除 hit class 並重置位置
            } 
